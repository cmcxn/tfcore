...
void BattlegroundsBot::Update()
{
    if (sGameEventMgr.IsActiveEvent(EVENT_BATTLEGROUNDSBOT))
    {
        sGameEventMgr.StopEvent(EVENT_BATTLEGROUNDSBOT, true);
        StopBot();
    }
    else
    {
        time_t currenttime = time(nullptr);
        if (sGameEventMgr.CheckOneGameEvent(EVENT_BATTLEGROUNDSBOT, currenttime))
        {
            sGameEventMgr.StartEvent(EVENT_BATTLEGROUNDSBOT, true);
            StartBot();
        }
    }
}

void BattlegroundsBot::StartBot()
{
    //sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BattlegroundsBot::StartBot()");
    sPlayerBotMgr.setAllowBotEnterBG(true);
}

void BattlegroundsBot::StopBot()
{
    //sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BattlegroundsBot::StopBot()");
    sPlayerBotMgr.setAllowBotEnterBG(false);
}

uint32 BattlegroundsBot::GetNextUpdateDelay()
{
    return sGameEventMgr.NextCheck(EVENT_BATTLEGROUNDSBOT);
}
...
void GameEventMgr::LoadHardcodedEvents(HardcodedEventList& eventList)
{
    ...
    auto battlegroundsBot = new BattlegroundsBot();
    eventList = { invasion, nightmare, darkmoon, fireworks, goblets, scourge_invasion, war_effort, battlegroundsBot };
}
