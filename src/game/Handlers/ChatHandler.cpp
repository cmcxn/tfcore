...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recv_data)
{
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_SAY:
        {
            ...
            if (!sEluna->OnChat(GetPlayer(), type, lang, msgStr))
                return;
#endif /* ENABLE_ELUNA */
...
#ifdef ENABLE_ELUNA
            std::string msgStr = msg;
            if (!sEluna->OnChat(GetPlayer(), type, LANG_UNIVERSAL, msgStr))
                return;
#endif /* ENABLE_ELUNA */
...
#ifdef ENABLE_ELUNA
                    std::string msgStr = msg;
                    if (!sEluna->OnChat(masterPlr->GetSession()->GetPlayer(), type, lang, msgStr, toPlayer))
                        return;
#endif /* ENABLE_ELUNA */
...
#ifdef ENABLE_ELUNA
            std::string msgStr = msg;
            if (!sEluna->OnChat(GetPlayer(), type, lang, msgStr, group))
                return;
#endif /* ENABLE_ELUNA */
            WorldPacket data;
...
#ifdef ENABLE_ELUNA
                    std::string msgStr = msg;
					if (!sEluna->OnChat(GetPlayer(), type, lang, msgStr, guild))
						return;
#endif /* ENABLE_ELUNA */
