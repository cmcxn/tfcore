            ...
            if (resultMail)
            {
                do
                {
                    ...
                    if (has_items)
                    {
                        ...
                        QueryResult* resultItems = CharacterDatabase.PQuery("SELECT `creator_guid`, `gift_creator_guid`, `count`, `duration`, `charges`, `flags`, `enchantments`, `random_property_id`, `durability`, `text`, `item_guid`, `item_instance`.`item_id`, `generated_loot`, `loot_trade_expire`, `loot_trade_players` FROM `mail_items` JOIN `item_instance` ON `item_guid` = `guid` WHERE `mail_id`='%u'", mail_id);
                        if (resultItems)
                        {
                            do
                            {
            ...
            if (!item->HasGeneratedLoot())
            {
                ...
                switch (loot_type)
                {
                    case LOOT_DISENCHANTING:
                        ...
                        break;
                    default:
                        //sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "GenerateMoneyLoot");
#ifdef ENABLE_ELUNA
                        sEluna->OnOpenBox(GetSession()->GetPlayer(), item);
#endif /* ENABLE_ELUNA */
...
void Player::SetVisibleItemSlot(uint8 slot, Item* pItem)
{
    ...
    sT->OnAfterPlayerSetVisibleItemSlot(this, slot, pItem);
}
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= nullptr*/, uint32 spellid /*= 0*/, bool nocheck)
{
    ...
    if (m_session->GetPlayer()->CanUseDonation(3))
    {
...
void Player::_LoadAlternativeSpec() {
    ...
    QueryResult* result = CharacterDatabase.PQuery("SELECT altspells FROM v_character_altspec WHERE guid = '%u'", GetGUIDLow());
...
void Player::_SaveAlternativeSpec()
{
    ...
    std::ostringstream ss;
    ss << "REPLACE INTO v_character_altspec (guid, altspells) VALUES ('" << GetGUIDLow() << "', '";
