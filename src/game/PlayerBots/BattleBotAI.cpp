...
void BattleBotAI::UpdateAI(uint32 const diff)
{
    ...
    if (!me->InBattleGround())
    {
        ...
        {
            BattleGroundQueue& bgQueue = sBattleGroundMgr.m_battleGroundQueues[m_battlegroundId];
            //sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BattleGroundQueue() %d", bgQueue.m_queuedPlayers.size());

            // 如果不在允许机器人加入战场的时间内或者战场没有人排队，就忽略
            if (!sPlayerBotMgr.getAllowBotEnterBG() || bgQueue.m_queuedPlayers.size() < 1)
                return;
            //sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "JoinBattleGroundQueue()");
