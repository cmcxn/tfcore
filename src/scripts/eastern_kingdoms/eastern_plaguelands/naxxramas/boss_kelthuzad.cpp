...
static constexpr uint32 NUM_UNDEAD_SPAWNS = 7;
            ...
            switch (eventId)
            {
                ...
                case EVENT_SKELETON:
                {
                    if (numSkeletons < 120)
                    {
                        ...
                        {
                            uint32 repeat_next = std::max(uint32(3750 - 25 * numSkeletons), uint32(3000));
    ...
    void UpdateP2P3(uint32 diff)
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                {
                    events.Repeat(Seconds(urand(15, 17))); // todo: this is guesswork
